...
import dev.ikm.tinkar.forge.wrapper.DescriptionTextMethodWrapper;
import dev.ikm.tinkar.forge.wrapper.ParentOfNavigationMethodWrapper;
...
public class TinkarForge implements Forge {
    ...
    private Stream<Entity<? extends EntityVersion>> entities;
    ...
    public Forge config(Path templatesDirectory) {
        return config(config -> {
            ...
            config.setSharedVariable("parent", new ParentOfNavigationMethodWrapper(navigationCalculator));
    ...
    @Override
    public Forge data(Stream<Entity<? extends EntityVersion>> entities) {
        dataModel.put("entities", entities.iterator());
    ...
    public Forge template(String templateName, Writer writer) {
        ...
        try {
            ...
            LOG.error("Template wasn't formatted correct!", e);
            throw new RuntimeException(e);
        }
        return this;
    }
